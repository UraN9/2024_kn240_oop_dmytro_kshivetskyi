...
Для роботи з сторонніми бібліотеками їх потрібно спочатку встановити. Для їх встановлення є інструмент PIP (Python Install Package). Щоб його встановити ми його встановили та потім переконалися чи він працює:
...
Після чого ми встановили першу посторонню бібліотеку _**requests**_:

```bash
pip install requests
```

Та опробували її в пайтон інтерпретаторі
```python
import requests
...
Після виправлення помилки я запустив [цю програму](anime.py), запустився сервер за посиланням _http://127.0.0.1:5000/_, результат можна подивитись за наступним зображенням:
...
Середовища також можна параметризувати за допомогою змінних середовища (Environment Variables). Для цього у папці повинен буди файл **.env** із заданими змінними у форматі **KEY=VALUE**. Pipenv автоматично розпізнає ці файли та робить їх доступними всередині середовища. Створив файл **.env** та виконав наступний код:
```python
import os
os.environ['HELLO']
```
